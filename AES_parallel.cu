...
void first_try(const uint8_t *sbox, const struct AES_ctx *ctx, uint8_t *buf, size_t length)
{
    ...
    size_t stride = 1;
    size_t AES_num_block = length / AES_BLOCKLEN;   //số khối AES
    size_t threadPerBlock = min((AES_num_block + stride - 1) / stride, (size_t)iProp.maxThreadsPerBlock);
